...
wget -O ${url_save_file} 'http://172.21.0.99:8080/intf/developer/getNewApkLog.jsp?minute=60&type=1&size=300' >>  ${log_file}  2>&1
		...
		do
		  ...
		  /usr/bin/rsync -avzPu  --bwlimit=30000 --timeout=10 ${backup_dir}/${Date_dir}/pconline${Date_file} ${i}::ftp-new-apk/${Date_dir}/ >>  ${log_file}  2>&1
			...
			[ $? -ne 0 ] && /usr/bin/rsync -avzPu  --bwlimit=15000 --timeout=10 ${backup_dir}/${Date_dir}/pconline${Date_file} ${i}::ftp-new-apk/${Date_dir}/ >>  ${log_file}  2>&1
                ...
                echo "curl -I http://172.21.0.99:8080/sync/apk/result.do?fileName=$line" >> /data1/sh/apk/log/curl.log
                curl  http://172.21.0.99:8080/sync/apk/result.do?fileName=$line >> /data1/sh/apk/log/curl.log
